...
$result = makeQuery(makeConn(),"SELECT * FROM `products` WHERE `category_1`='$cat' ORDER BY `date_create` DESC LIMIT $limit");
	recommendedProducts($result);
}

function recommendedSimilar($cat,$id=0,$limit=3) {
$result = makeQuery(makeConn(),"SELECT * FROM `products` WHERE `category_1`='$cat' AND `id` <>$id ORDER BY rand() LIMIT $limit");
	recommendedProducts($result);
}